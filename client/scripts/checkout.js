...
async function calculateTotal(){
    ...
    cart.forEach(async (item)=>{
        try {
            const foodDetail = await getFood(item.foodID);
...
async function loadCheckoutPage(){
    ...
    cart.forEach(async (item) => {
        try {
            ...
            console.error('Unexpected error rendering checkout food items')
        }
    ...
    const totalBeforeTax = await calculateTotal()
